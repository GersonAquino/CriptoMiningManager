...
namespace CriptoMiningManager.Views.UserControls.Configuracoes.Editores
{
	public partial class ConfiguracaoGeralEditorUserControl : XtraUserControl
	{
		private bool EditarConfigAtiva { get; }
		...
		private async void GravarBBI_ItemClick(object sender, DevExpress.XtraBars.ItemClickEventArgs e)
		{
			try
			{
				...
				if (EditarConfigAtiva && !Entidade.Ativo &&
					!MessageBoxesHelper.PerguntaSimples("Está a desativar a Configuração Geral ativa, pretende continuar?", "Aviso", MessageBoxIcon.Warning))
					return;
				...
				if (idGerado != -1)
				{
					if (Entidade.Ativo)
					{
						...
						MessageBoxesHelper.MostraInformacao("Configuração Geral Ativa gravada com sucesso.", "Sucesso");
					}
					else if (EditarConfigAtiva)
					{
						Global.ConfigGeralAtiva = null;
						MessageBoxesHelper.MostraAviso("Configuração Geral Ativa desativada com sucesso. É altamente recomendado ativar uma nova Configuração Geral!");
					}
					else
					...
					if (this.Parent is DocumentContainer docContainer)
					{
						...
						else
						{
							...
							docContainer.Document.Caption = $"Editar {Entidade}";
						}
					}
				}
				else
